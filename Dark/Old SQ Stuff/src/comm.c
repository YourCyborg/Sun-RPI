...
int main( int argc, char **argv )
{
  ...
  /*
    ...
    boot_db( );
    sprintf( log_buf, "Darksun MUD up and running on port %d at %s", port,
    (char *) ctime ( &current_time ));
    log_string( log_buf );

    sprintf( log_buf, "%s", (char *) ctime ( &current_time ) );
    append_stat(  STAT_FILE, "Running" );
    append_stat(  BOOT_FILE, log_buf );
...
*/      
            if ( wizlock && downtime )
            {
                ...
                write_to_buffer( dnew, "Enable VT100 text (n) ?\n\n ", 0 );
                return;
            }
            else if ( wizlock )
            {
...
*/
         ...
         case 'M' :
            ...
            i = buf2; break;
         case 't' :
            sprintf( buf2, "%d", ch->stun );
            i = buf2; break;
         case 'T' :
            sprintf( buf2, "%d", ch->max_stun );
            i = buf2; break;
         case 'v' :
...
/*    int col, sn; */
	...
	if ( ch->level == 0 )
	{
	    ...
	    ch->level	= 1;
	    ch->max_hit = race_array[ch->race]->hits_base + dice( race_array[ch->race]->hitnodice, race_array[ch->race]->hitsizedice );
	    ch->hit[HIT_LOC_BODY]	= ch->max_hit;
	    ch->max_mana = race_array[ch->race]->mana_base + dice( race_array[ch->race]->mananodice, race_array[ch->race]->manasizedice );
	    ch->mana	= ch->max_mana;
	    ch->max_move = race_array[ch->race]->move_base + dice( race_array[ch->race]->move_nodice, race_array[ch->race]->movesizedice + ch->perm_stat_phys_dex);
	    ch->move	= ch->max_move;
            ch->stun    = ch->max_stun;
            ch->max_stun =  race_array[ch->race]->stun_base + dice( race_array[ch->race]->move_nodice, race_array[ch->race]->movesizedice + ch->perm_stat_phys_tou );
	    ...
	    ch->weight = race_array[ch->race]->weight_base
            ...
            /* Humanoid or Human */
            if ( ch->race == 0 || ch->race == 1 || ch->race == 7 || ch->race == 8){
            ch->pcdata->learned[gsn_andean] = 100;
            ch->language = 0;
            char_to_room( ch, get_room_index( ROOM_VNUM_CHARACTER_GENERATION ), FALSE );
            do_look(ch, "auto");
            do_classupdate(ch);
            return;
            }
            ...
            ch->pcdata->learned[gsn_luderwen] = 100;
            ch->pcdata->learned[gsn_hide] = 40;
            ch->pcdata->learned[gsn_sneak] = 35;
            ch->language = 1;
            char_to_room( ch, get_room_index( ROOM_VNUM_CHARACTER_GENERATION ), FALSE );
            do_look(ch, "auto");
            do_classupdate(ch);
            ...
            ch->pcdata->learned[gsn_vedican] = 100;
            ch->language = 2;
            char_to_room( ch, get_room_index( ROOM_VNUM_CHARACTER_GENERATION ), FALSE );
            do_look(ch, "auto");
            do_classupdate(ch);
            ...
            ch->pcdata->learned[gsn_jikkean] = 100;
            ch->pcdata->learned[gsn_hide] = 40;
            ch->pcdata->learned[gsn_sneak] = 35;
            ch->language = 3;
            char_to_room( ch, get_room_index( ROOM_VNUM_CHARACTER_GENERATION ), FALSE );
...
/*
        ...
        {
            write_to_buffer( d, "\n\rYeah right.\n\r", 0);
            write_to_buffer( d, "This attempt has been logged.\n\r", 0 );
            sprintf( log_buf, "%s tried creating Delerak.\n\r", d->host );
            log_string( log_buf );
            close_socket(d);
            return;
        }*/
	...
	default:
        ...
        write_to_buffer( d, "Race options available:\n\nHuman, Half-elf, half-giant, thri-kreen\n\rMul, Gith\n\r", 0 );
	write_to_buffer( d, "Select a race: ", 0 );
	d->connected = CON_CREATE_NEW_RACE;
	break;

    case CON_CREATE_NEW_RACE:
	    /* KLUDGE */
	for ( iRace = 1; iRace <= 8; iRace++ )
	{
...
//        SET_BIT( ch->act, PLR_HOLYLIGHT );
	...
	log_string( log_buf );
        free_char( ch );
	d->character = NULL;
        write_to_buffer( d, help_greeting3, 0 );
	d->connected = CON_MAIN_MENU;
	break;
    }
