...
char *attack1, *attack2, *attack3, *attack4, *attack5, *attack6, *attack7, 
*attack8, *attack9, *attack10;
...
*/
  ...
  if ( fMessage == TRUE )
  {
    ...
    case POS_DEAD:
	act( "$n collapses to the ground in a heap. \a", victim, NULL, NULL, NULL, NULL, TO_ROOM, SENSE_SIGHT );
	send_to_char( "You have been killed!\a\n\r\n\r", victim );
	break;
...
/* need to add death penalty */
    ...
    if( IS_NPC(victim) )
    {
    sprintf(buf, "%s killed %s[npc] at %d.",ch->name, victim->short_descr, ch->in_room->vnum );
    }
    else if ( IS_NPC(ch) )
    {
    sprintf(buf, "%s[npc] killed %s at %d.",ch->short_descr, victim->name, ch->in_room->vnum );
    }
    else
    {
    sprintf(buf, "%s killed %s at %d.",ch->name, victim->name, ch->in_room->vnum );
    }
...
void maybe_set_fighting( CHAR_DATA *ch, CHAR_DATA *victim )
{
  ...
  if ( IS_NPC(ch) ){
  sprintf(buf, "%s[npc] attacked %s in room %d.", ch->short_descr, victim->name, ch->in_room->vnum );
  }
  else if ( IS_NPC(victim)){
  sprintf(buf, "%s attacked %s[npc] in room %d.", ch->name, victim->short_descr, ch->in_room->vnum );
  }
  else {
  sprintf(buf, "%s attacked %s in room %d.", ch->name, victim->name, ch->in_room->vnum );
  }
...
/*    OBJ_DATA *obj;*/
    ...
    if ( IS_SET(victim->position, POS_SITTING ) )
    {
       send_to_char("Yeah right. Get them up first.\n\r", ch);
       return;
    }
...
void do_rescue( CHAR_DATA *ch, char *argument )
{
    ...
    act( "BANZAI...",  ch, victim, NULL, NULL, NULL, TO_CHAR, SENSE_SIXTH );
    act( "$n rescues you!  You are confused.", ch, victim, NULL, NULL, NULL, TO_VICT, SENSE_SIGHT|SENSE_HEARING );
    act( "$n rescues $N!",  ch, victim, NULL, NULL, NULL, TO_NOTVICT, SENSE_SIGHT );
